...
Runtime-loaded by `scripts/run_term_injection.py` at §4a step 8 (every
...
attestation. `semantic_model_consumed` (Layer A; v3.6 §22.7) is the
list of raw `table_name` rows you consulted. `dbt_semantic_model_consumed`
(Layer B; v3.7 §23.7) is the list of dbt `model_name` rows you consulted.
...
**4. Scope sanity (§2d).** Does the frozen `scope_tables` list make
...
**5. Shadow rubric (§5b instrumentation, not used for stopping).**
...
MUST be ≤ 80 words (hard cap — v3.6 Prereq B):
...
PARTIAL counting 0.5. If `total_count==0`, emit 50 (v3.1 §11 Q5 fallback).
