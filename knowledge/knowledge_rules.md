...
SQL that compiles and runs is not proof it measures what the business term claims to measure. Phase 13 retry fixes parseable column-not-found errors. Phase 14 adds a second gate between `dbt run` success and `dbt test`: an LLM validator checks the model against the term across three **deterministic** dimensions:
