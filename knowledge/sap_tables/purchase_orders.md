...
- **#68** (2026-04-19) — source_column_roles_seed_and_classifier: Pipeline is wired and verified dry-run against live DB without burning LLM tokens. Cold-start execution is gated behind explicit CLI invocation (python scripts/classify_source_columns.py) so the ~$0.18 / ~15-min burn only happens when the user is ready. Once cold-start runs, (a) bump _csv_safeguard floor from 10 to ~280 (99% of observed cold-start row count), (b) wire an optional delta invocation inside end_of_task.py (currently manual) with the §1b 50-row cap intact.
